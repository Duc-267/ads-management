...
export async function forgotPassword(userData: IForgotPasswordRequest): Promise<Boolean | IServerError> {
  try {
...
export async function verifyToken(token: string): Promise<IVerifyTokenResponse> {
  try {
    const response = await api.post(`/auth/verify-token`, JSON.stringify(token), {
      headers: {
